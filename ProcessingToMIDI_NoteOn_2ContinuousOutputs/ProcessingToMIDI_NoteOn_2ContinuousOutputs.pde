...
void draw() {
  int note = notesInOctave[int(random(8))];
  myBus.sendNoteOn(0, note, (int)velocity); // Send a Midi noteOn
   //float sine = sin(frameCount*.1)*50;
   delay(int(pitch*5));
  background(0);  
  drawText();
  drawCircle();
...
void oscEvent(OscMessage theOscMessage) {
 if (theOscMessage.checkAddrPattern("/wek/outputs")==true) {
   //println(theOscMessage.get(0), theOscMessage.get(1));
     if(theOscMessage.checkTypetag("ff")) { //Now looking for 2 parameters
        pitch = theOscMessage.get(0).floatValue(); //get this parameter
        velocity = theOscMessage.get(1).floatValue(); //get 2nd parameter
